<template>
    <form @submit.prevent="handleSubmit" class="space-y-6 w-3/4 max-w-lg">
        ...
        <div class="mt-1">
            <label for="title" class="text-gray-700 text-lg">タイトル</label>
            <input v-model="post.title" type="text" id="title"
              class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm placeholder-gray-400 border border-gray-300 rounded-md"
              placeholder="Railsチュートリアル1章を完了">
        </div>
        <div class="mt-1">
          <label for="content" class="text-gray-700 text-lg">本文</label>
          <textarea v-model="post.content" rows="5" id="content"
            class="shadow-sm focus:ring-indigo-500 focus:border-indigo-500 mt-1 block w-full sm:text-sm placeholder-gray-400 border border-gray-300 rounded-md"
            placeholder="環境構築を無事に終えることができた！"></textarea>
        </div>
         <div class="mt-1">
            <label for="tag_names" class="text-gray-700 text-lg">タグ</label>
          ...
          <div class="px-4 py-3 text-right sm:px-6">
            <button type="submit"
             ...
             <router-link :to="`/posts/${post.id}`"
                  class="inline-flex justify-center py-2 px-4 ml-3 border border-transparent shadow-sm text-sm font-medium rounded-md text-gray-700 bg-gray-300 hover:bg-gray-400 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500">キャンセル</router-link>
        </div>
    </form>
